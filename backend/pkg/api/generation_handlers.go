...
var (
	...
	markdownStrong   = regexp.MustCompile(`\*\*([\s\S]+?)\*\*`)
	markdownEmphasis = regexp.MustCompile(`\*([\s\S]+?)\*`)
	markdownMarker   = regexp.MustCompile(`[*_` + "`" + `]+`)
	markdownHeading  = regexp.MustCompile(`^(#{1,6})\s+(.+)$`)
	// Fenced code opener with an optional language (``` or ~~~).
	markdownFence    = regexp.MustCompile("^(`{3,}|~{3,})\\s*([A-Za-z][\\w-]*)?\\s*$")
	verseLang        = regexp.MustCompile(`(?i)^(verse|poem|poetry|stanza|stih|stikh)$`)
...
func markdownBlocks(markdown string) []markdownBlock {
	...
	for i := 0; i < len(parts); i++ {
		...
		if fence := markdownFence.FindStringSubmatch(line); fence != nil {
			flush()
			marker := fence[1]
			isVerse := verseLang.MatchString(fence[2])
			fenced := []string{}
			i++
			for ; i < len(parts); i++ {
				if strings.HasPrefix(strings.TrimSpace(parts[i]), marker) {
					break
				}
				fenced = append(fenced, strings.TrimRight(parts[i], " \t"))
			}
			if isVerse {
				// One block per stanza (blank-line separated) so the paginator can
				// break a long song between stanzas, never mid-line.
				stanza := []string{}
				flushStanza := func() {
					if len(stanza) > 0 {
						out = append(out, markdownBlock{Kind: "verse", Text: strings.Join(stanza, "\n")})
						stanza = []string{}
					}
				}
				for _, vl := range fenced {
					if strings.TrimSpace(vl) == "" {
						flushStanza()
						continue
					}
					stanza = append(stanza, vl)
				}
				flushStanza()
			} else if len(fenced) > 0 {
				out = append(out, markdownBlock{Kind: "p", Text: strings.Join(fenced, " ")})
			}
			continue
		}
		if line == "" {
...
func markdownToManuscriptBlocks(markdown string, dropCap bool, settings generationSettings) []manuscriptBlock {
	...
	for _, block := range markdownBlocks(markdown) {
		...
		case "verse":
			...
			blocks = append(blocks, manuscriptBlock{HTML: renderVerseBlock(block.Text), Units: verseUnits(block.Text), Kind: "text"})
		default:
...
// renderVerseBlock renders a ```verse fence as centered stanzas, preserving
// each line break. Stanzas are blank-line separated within the fence.
func renderVerseBlock(text string) string {
	var b strings.Builder
	b.WriteString(`<div class="manuscript-verse">`)
	for _, stanza := range strings.Split(strings.TrimRight(text, "\n"), "\n\n") {
		...
		for _, line := range lines {
			...
			rendered = append(rendered, renderInlineMarkdown(strings.TrimSpace(line)))
		}
		if len(rendered) == 0 {
			continue
		}
		b.WriteString(`<span class="manuscript-stanza">` + strings.Join(rendered, "<br>") + `</span>`)
	}
	b.WriteString(`</div>`)
	return b.String()
}

func verseUnits(text string) float64 {
	lines := strings.Count(strings.TrimSpace(text), "\n") + 1
	return maxFloat(6, float64(lines)*2.6+3)
}
